...
// AES-256-GCM encryption for GitHub OAuth tokens at rest. Key comes from
// GITHUB_TOKEN_ENCRYPTION_KEY (32 raw bytes, base64-encoded) — never
...
// String field. Deliberately near-identical to utils/googleTokenCrypto.js.

function getKey() {
  const raw = process.env.GITHUB_TOKEN_ENCRYPTION_KEY;
  if (!raw) {
    throw new Error('GITHUB_TOKEN_ENCRYPTION_KEY is not set — cannot encrypt/decrypt GitHub tokens.');
  }
